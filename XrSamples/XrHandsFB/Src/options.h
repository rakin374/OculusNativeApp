...
inline XrFormFactor GetXrFormFactor(const std::string& formFactorStr) {
    ...
    Log::Write(Log::Level::Verbose, (Fmt("Unknown form factor '%s'", formFactorStr.c_str())));
    exit(1);
}
...
inline XrViewConfigurationType GetXrViewConfigurationType(const std::string& viewConfigurationStr) {
    ...
    Log::Write(Log::Level::Verbose, (Fmt("Unknown view configuration '%s'", viewConfigurationStr.c_str())));
    exit(1);
...
inline XrEnvironmentBlendMode GetXrEnvironmentBlendMode(const std::string& environmentBlendModeStr) {
   ...
   Log::Write(Log::Level::Verbose,(Fmt("Unknown environment blend mode '%s'", environmentBlendModeStr.c_str())));
    exit(1);
}
...
inline const char* GetXrEnvironmentBlendModeStr(XrEnvironmentBlendMode environmentBlendMode) {
    switch (environmentBlendMode) {
        ...
        case XR_ENVIRONMENT_BLEND_MODE_ALPHA_BLEND:
            return "AlphaBlend";
        default:
            Log::Write(Log::Level::Verbose,(Fmt("Unknown environment blend mode '%s'", to_string(environmentBlendMode))));
            exit(1);
    }
}
